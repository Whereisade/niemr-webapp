// app/patient/imaging/[id]/page.js
"use client";
...
export default function PatientImagingDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadRequest() {
      try {
        ...
        console.error("Failed to load patient imaging request", err);
        if (!cancelled) {
          setError(
            err?.message ||
  ...
  useEffect(() => {
    ...
    async function loadAttachments() {
      try {
        ...
        console.error("Failed to load imaging attachments (patient)", err);
        if (!cancelled) {
          setAttachmentsError(
            err?.message ||
    ...
    <main className="mx-auto max-w-4xl space-y-6 p-6 md:p-10">
      ...
      {!loading && request && (
        <section className="space-y-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
              Why this imaging was requested
            </p>
          ...
          <div className="space-y-2">
            ...
            {!attachmentsLoading &&
              ...
              attachments.length === 0 && (
                <p className="text-xs text-slate-500">
                  No files attached to this imaging test yet.
                </p>
              )}
