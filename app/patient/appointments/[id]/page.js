...
export default async function PatientAppointmentDetail({ params }) {
  const { id } = params || {};
  ...
  if (!appt) {
    return (
      <main className="mx-auto max-w-4xl p-6 md:p-10">
        ...
        <a href="/patient/appointments" className="mt-6 inline-block text-blue-700 hover:underline">
      ...
      <header className="mb-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-slate-600">Patient view</p>
          </div>
      ...
      <section className="grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <p className="text-sm text-slate-700 whitespace-pre-wrap">
            {appt.patient_note || appt.notes || "—"}
          </p>
        </div>
      </section>

      <div className="mt-8">
        <a href="/patient/appointments" className="inline-block text-blue-700 hover:underline">
          ← Back to appointments
        </a>
      </div>
    </main>
