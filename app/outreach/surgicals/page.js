...
export default function OutreachSurgicalsPage() {
  const {
  ...
  const enabled = isModuleEnabled(modulesEnabled, "surgicals");
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.SURGICALS_CREATE);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="inline-flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Surgicals</h1>
          </div>
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record procedures done.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="surgicals" />
      ) : null}
    </div>
  );
}
