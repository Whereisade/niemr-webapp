...
export default function OutreachVitalsPage() {
  const {
  ...
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.VITALS_CREATE);
  const enabled = isModuleEnabled(modulesEnabled, "vitals");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Vitals</h1>
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record or view vitals for this outreach.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="vitals" />
      ) : null}
    </div>
  );
}
