...
export default function OutreachBloodDonationPage() {
  const {
  ...
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.BLOOD_CREATE);
  const enabled = isModuleEnabled(modulesEnabled, "blood_donation");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Blood Donation</h1>
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record blood donation screening/outcomes.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="blood" />
      ) : null}
    </div>
  );
}
