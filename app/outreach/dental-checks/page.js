...
export default function OutreachDentalChecksPage() {
  const {
  ...
  const enabled = isModuleEnabled(modulesEnabled, "dental_checks");
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.DENTAL_CHECKS_CREATE);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="inline-flex items-center gap-2">
            <Stethoscope className="h-5 w-5 text-slate-800" />
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Dental checks</h1>
          </div>
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record dental checks.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="dental_checks" />
      ) : null}
    </div>
  );
}
