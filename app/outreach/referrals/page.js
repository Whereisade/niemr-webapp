...
export default function OutreachReferralsPage() {
  const {
  ...
  const enabled = isModuleEnabled(modulesEnabled, "referral");
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.REFERRAL_CREATE);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="inline-flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Referral</h1>
          </div>
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record referrals.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="referral" />
      ) : null}
    </div>
  );
}
