...
export default function OutreachLabPage() {
  ...
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.LAB_ORDER_CREATE);
  const canSeeCatalog = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.LAB_CATALOG_VIEW);
  const enabled = isModuleEnabled(modulesEnabled, "lab");
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Lab</h1>
            <p className="mt-1 text-sm text-slate-600">Pick a patient to create lab orders or enter results.</p>
          </div>
        </div>

        {canSeeCatalog ? (
          <Link
            href="/outreach/labs/catalog"
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="lab" />
      ) : null}
    </div>
  );
}
