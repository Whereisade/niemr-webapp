...
export default function OutreachEncountersPage() {
  const {
  ...
  const allowed = isOutreachSuperAdmin || hasPerm(permissions, OUTREACH_PERMS.ENCOUNTER_CREATE);
  const enabled = isModuleEnabled(modulesEnabled, "encounter");
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Pick a patient to record clinical encounters for this outreach.</p>
        </div>
      </div>
      ...
      {selectedEventId && enabled && allowed ? (
        <PatientPicker eventId={selectedEventId} tabKey="encounter" />
      ) : null}
    </div>
  );
}
