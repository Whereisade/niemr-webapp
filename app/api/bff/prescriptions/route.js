// app/api/bff/prescriptions/route.js
...
export async function GET(req) {
  try {
    ...
    // 2) Build backend URL: http://localhost:8000/api/pharmacy/prescriptions/?...
    const incomingUrl = new URL(req.url);
    const search = incomingUrl.search || ""; // includes leading "?" if present
    const path = withTrailingSlash("/api/pharmacy/prescriptions");
    ...
    return NextResponse.json(
      {
        detail: `BFF error while calling prescriptions: ${
