// app/api/bff/notifications/route.js
...
export async function GET(req) {
  try {
    ...
    // 2) Build backend URL: http://localhost:8000/api/notifications/?...
    const incomingUrl = new URL(req.url);
    const search = incomingUrl.search || ""; // includes leading "?" if present
    const path = withTrailingSlash("/api/notifications");
    ...
    return NextResponse.json(
      {
        detail: `BFF error while calling notifications: ${
