// app/api/bff/billing/ledger/route.js
...
export async function GET(req) {
  try {
    ...
    // 2) Build backend URL: http://localhost:8000/api/billing/charges/ledger/?...
    const incomingUrl = new URL(req.url);
    const search = incomingUrl.search || ""; // includes leading "?" if present
    const path = withTrailingSlash("/api/billing/charges/ledger");
    ...
    return NextResponse.json(
      {
        detail: `BFF error while calling billing ledger: ${
