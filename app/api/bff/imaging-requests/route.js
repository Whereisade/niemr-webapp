// app/api/bff/imaging-requests/route.js
...
export async function GET(req) {
  try {
    ...
    // 2) Build backend URL: http://localhost:8000/api/imaging/requests/?...
    const incomingUrl = new URL(req.url);
    const search = incomingUrl.search || ""; // includes leading "?" if present
    const path = withTrailingSlash("/api/imaging/requests");
    ...
    return NextResponse.json(
      {
        detail: `BFF error while calling imaging requests: ${
