...
export default async function FacilityAppointmentsPage({ searchParams: spPromise }) {
  ...
  const { items, next, prev } = await fetchList(`/appointments/${qs({ page, limit: 20, date, status, provider, q })}`);

  return (
    <main className="mx-auto max-w-7xl p-6 md:p-10">
      <header className="mb-6">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-900">Facility Appointments</h1>
        <p className="mt-1 text-slate-600">All appointments across your facility.</p>
      </header>

      <form className="mb-5 grid gap-3 md:grid-cols-5" method="GET">
        <input name="q" defaultValue={q} placeholder="Search patient/reason" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" />
        ...
        <input name="provider" defaultValue={provider} placeholder="Provider name (optional)" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" />
      ...
      <nav className="mt-4 flex items-center justify-between">
        <a href={`?${new URLSearchParams({ q, date, provider, status, page: String(Math.max(1, prev || 1)) }).toString()}`}
           className={`text-sm ${prev ? "text-blue-700 hover:underline" : "text-slate-400 pointer-events-none"}`}>← Previous</a>
        <a href={`?${new URLSearchParams({ q, date, provider, status, page: String(next || page) }).toString()}`}
