  ...
  return (
    <main className="mx-auto max-w-7xl p-6 md:p-10">
      <header className="mb-8">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-900">Provider Home</h1>
      ...
      <section className="grid gap-4 md:grid-cols-3 mb-8">
        ...
        <a href="/provider/appointments"
          ...
          <p className="text-sm text-slate-700">Go to schedule</p>
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-3">
          ...
          <a href="/provider/appointments" className="text-sm text-blue-700 hover:underline">View all</a>
        </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {appts.length ? appts.map(a => (
                ...
                <tr><td colSpan={4} className="px-4 py-6 text-slate-600">No appointments for today.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <a href="/provider/notifications" className="text-sm text-blue-700 hover:underline">View all</a>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <ul className="divide-y divide-slate-100">
            {notifList.length ? notifList.slice(0, 6).map((n, i) => (
              ...
              <li className="py-3 text-slate-600">You’re all caught up.</li>
            )}
          </ul>
        </div>
      </section>
