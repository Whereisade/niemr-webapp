...
export default function ProviderPharmacyCatalogPage() {
  ...
  const isPharmacyProvider = meRole === "PHARMACY";

  // --- load catalog for ALL providers (read-only for non-pharmacy) ---
  ...
  // --- create single drug (PHARMACY providers only) ---
  async function handleCreate(e) {
    e.preventDefault();
    if (!isPharmacyProvider) return; // hard-guard on client
    setCreateError(null);
    setCreating(true);
    try {
      const payload = {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (errJson && (errJson.detail || errJson.non_field_errors)) {
            msg = errJson.detail || errJson.non_field_errors.join(", ");
          }
        } catch {}
        throw new Error(msg);
      }
  ...
  // --- import CSV (PHARMACY providers only) ---
    ...
    return (
      <main className="mx-auto max-w-3xl p-6 md:p-10">
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800">
          You must be logged in as a provider to view the drug catalog.
        </div>
      </main>
    );
  }

  const headerBadgeText = isPharmacyProvider
    ? "Independent Pharmacy · Catalog"
    : "Provider · Drug catalog";

  const headerSubtitle = isPharmacyProvider
    ? "Manage the list of drugs you can prescribe and dispense as an independent pharmacist."
    : "Browse the shared drug catalog. Contact pharmacy if you need new items added or updated.";
  ...
  return (
    <main className="relative mx-auto max-w-6xl space-y-6 p-6 md:p-10">
      ...
      {/* Layout: left = list + search, right = import + create (pharmacists only) */}
      <section className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)]">
        ...
        {/* Right column: CSV import + create — ONLY for pharmacy providers */}
        {isPharmacyProvider && (
