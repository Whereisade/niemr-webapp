...
export async function uploadAttachments({
  ...
  for (const file of files) {
    ...
    if (patient) form.append("patient", String(patient));
    if (visibility) form.append("visibility", String(visibility));
    if (tag) form.append("tag", String(tag));
    if (description) form.append("description", String(description));

    if (patient !== undefined && patient !== null && String(patient).length) {
      form.append("patient", String(patient));
    }
    if (visibility) {
      form.append("visibility", String(visibility));
    }
    if (tag) {
      form.append("tag", String(tag));
    }
    if (description) {
