 ...
 *    end: "2025-11-30",    // optional
 *    s: "amoxicillin",     // optional search
 *  });
 *
...
export function usePrescriptions(params = {}) {
  ...
  const fetchNow = async (signal) => {
    ...
    setError(null);
    try {
      const url = `/api/bff/prescriptions${query ? `?${query}` : ""}`;
  ...
  const key = `/api/bff/prescriptions?${query}`;
